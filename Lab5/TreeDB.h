...
private:
    ...
    //use to insert node
    TreeNode* insertNode(TreeNode* root, DBentry* newEntry, int* x);

    //search the tree for the key
    TreeNode* findHelper(TreeNode* root, string name);

    //used to search for node to remove
    //if parent == nullptr, we know runner is at this-> root!!!
    //parent always follows runner
    bool removeHelper(TreeNode* parent, TreeNode* runner, string name);
    ...
    TreeNode* findLargest(TreeNode* parent, TreeNode* runner);
...
public:
    ...
    void inorder(ostream& out, TreeNode* root) const;
};
